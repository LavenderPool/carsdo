...
const hasEngines = computed(() => props.engines.data.length > 0);

const normalizedPaginationLinks = computed(() =>
    props.engines.links.map((link) => ({
        ...link,
        label: link.label
...
const remove = (id: number, name: string) => {
    if (!window.confirm(`Удалить двигатель "${name}"?`)) {
        return;
    }

    router.delete(route('admin.engines.destroy', id), {
        preserveScroll: true,
    });
};
</script>
        ...
        <template #header>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    <Link
                        :href="route('admin.engines.create')"
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="border-b border-gray-200 p-6">
                        <form class="flex flex-col gap-3 sm:flex-row" @submit.prevent="applySearch">
                            <input
                                v-model="search"
                                type="text"
                                placeholder="Поиск по названию, идентификатору или бренду"
                    ...
                    <div v-if="hasEngines" class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="engine in engines.data" :key="engine.id">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                        <div class="flex justify-end gap-4">
                                            <Link
                                                :href="route('admin.engines.edit', engine.id)"
                                            ...
                                            <button
                                                ...
                                                @click="remove(engine.id, engine.name)"
                                            >
                                                Удалить
                                            </button>
                                        </div>
                    ...
                    <div v-else class="p-6 text-sm text-gray-500">
                        Двигатели пока не созданы.
                    </div>
                </div>

                <div
                    v-if="engines.links.length > 3"
