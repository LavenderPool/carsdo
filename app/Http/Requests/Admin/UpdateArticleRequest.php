...
class UpdateArticleRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        foreach (AdminSeoFields::articleFields() as $field) {
            ...
            'body_json' => [
                ...
                function (string $attribute, mixed $value, \Closure $fail): void {
                    if ($this->boolean('_body_json_is_invalid')) {
                        $fail('Содержимое статьи повреждено. Откройте редактор заново и повторите сохранение.');

                        return;
                    }

                    if (! app(ArticleBodyRenderer::class)->hasMeaningfulContent(is_array($value) ? $value : null)) {
                        $fail('Добавьте текст статьи.');
                    }
                },
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'slug.required' => 'Укажите идентификатор статьи.',
            'slug.unique' => 'Статья с таким идентификатором уже существует.',
            'body_json.required' => 'Добавьте текст статьи.',
